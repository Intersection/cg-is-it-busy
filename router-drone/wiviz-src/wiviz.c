...
wiviz_host * gotHost(wiviz_cfg * cfg, u_char * mac, host_type type, int rssi);
...
void reloadConfig() {
  ...
  cnf = fopen("/jffs/usr/etc/wiviz-cfg", "r");
  if (!cnf) {
...
void dealWithPacket(wiviz_cfg * cfg, struct pcap_pkthdr * header, const u_char * packet) {
  ...
  host = gotHost(cfg, src, type, rssi);
  ...
  if (type == typeSta) {
    if (nonzeromac(bss)) {
      ...
      emergebss = gotHost(cfg, bss, typeAP, rssi);
...
//    fprintf(stdout, "%s\n", (type==typeAP) ? "AP" : ((type==typeSta) ? "Sta" : "Unk"));
	  if (type==typeSta) {
		  fprint_mac(stderr,mac, "|");
		  int divisor;
		  divisor = h->occupied ? 100 : 10;
		  fprintf(stderr,"-%i|", (h->RSSI * 9 / 10 + (-rssi * 10))/divisor);
		  fprintf(stderr,"%d\n",time(NULL));
	  }
 // }
  h->occupied = 1;
...
<<<<<<< HEAD:wtm-router/wiviz-src/wiviz.c
	printf("AP mac: ");
	print_mac(mac, "\n");
	if (!nonzeromac(mac)) return;
	wl_ioctl(WL_DEVICE, WLC_GET_AP, &ap, 4);
	if (ap) {
		host = gotHost(cfg, mac, typeAP, 0);
		...
		if (wl_ioctl(WL_DEVICE, WLC_GET_ASSOCLIST, macs, 4 + MAX_STA_COUNT * sizeof(ether_addr_t)) > -1) {
			for (i = 0; i < macs->count; i++) {
			  sta = gotHost(cfg, (char *)&macs->ea[i], typeSta, 0);
		...
		host = gotHost(cfg, mac, typeSta, 0);
		host->RSSI = 0;
